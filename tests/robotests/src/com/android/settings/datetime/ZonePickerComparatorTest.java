...
public class ZonePickerComparatorTest {
    ...
    private List<Map<String, Object>> getMockZonesList() {
        ...
        for (int i = 0; i < TESTNUM; i++) {
            zones.add(createMockDisplayEntry(tz, "GMT+08:00", mTestDisplayName[i], TESTNUM - i - 1));
        }
        return zones;
    }
    ...
    public void testComparator_sortString() {
        mSortKey = ZoneGetter.KEY_DISPLAY_LABEL;
        final ZonePicker.MyComparator comparator = new ZonePicker.MyComparator(mSortKey);
        assertThat(comparator).isNotNull();
        Collections.sort(mTestZonesList, comparator);
        for (int i = 0; i < TESTNUM; i++) {
            assertThat(mTestZonesList.get(i).get(mSortKey).toString())
    ...
    public void testComparator_sortInteger() {
        mSortKey = ZoneGetter.KEY_OFFSET;
        final ZonePicker.MyComparator comparator = new ZonePicker.MyComparator(mSortKey);   
        assertThat(comparator).isNotNull();
        Collections.sort(mTestZonesList, comparator);
        for (int i = 0; i < TESTNUM; i++) {
            assertThat(mTestZonesList.get(i).get(mSortKey)).isEqualTo(i);
        }
    }
}
