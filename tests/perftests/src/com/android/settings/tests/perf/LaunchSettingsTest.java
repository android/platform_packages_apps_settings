...
public class LaunchSettingsTest {
    ...
    private String getMax(String page) {
        if (mResult.get(page).size() == TEST_TIME) {
            return String.format("%s",
                    mResult.get(page).isEmpty() ? "null" : Collections.max(mResult.get(page)));
        }
        Log.i("SettingsPerfTests", String.format("Fail to get max of %s.", page));
        return "0";
    ...
    private String getMin(String page) {
        if (mResult.get(page).size() == TEST_TIME) {
            return String.format("%s",
                    mResult.get(page).isEmpty() ? "null" : Collections.min(mResult.get(page)));
        }
        Log.i("SettingsPerfTests", String.format("Fail to get min of %s.", page));
        return "0";
    }

    private String getAvg(String page) {
        if (mResult.get(page).size() == TEST_TIME) {
            return String.valueOf((int) mResult.get(page).stream().mapToInt(i -> i).average().orElse(0));
        }
        Log.i("SettingsPerfTests", String.format("Fail to get avg of %s.", page));
        return "0";
    }
    ...
    private String getPercentile(String page, double position) {
        ...
        Log.i("SettingsPerfTests", String.format("Fail to get percentile of %s.", page));
        return "0";
    }
}