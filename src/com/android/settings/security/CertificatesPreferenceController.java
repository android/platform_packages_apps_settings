...
//        mDeletePreference.setEnabled(!isEmpty() && !isRestricted());
//        Log.d(TAG, "isEmpty is " + isEmpty() + "isResticted is " + isRestricted());
    ...
    private boolean isRestricted() {
        if(mUserId == UserHandle.USER_SYSTEM) return false;
        final String restriction = UserManager.DISALLOW_CONFIG_CREDENTIALS;
        return RestrictedLockUtilsInternal.hasBaseUserRestriction(mContext, restriction,mUserId);
    }
    ...
    private class AliasLoader extends AsyncTask<Void, Void, List<Credential>> {
        ...
        protected List<Credential> doInBackground(Void... params) {
            Log.d(TAG, "AliasLoader doInBackground started");
//            try {
//                Thread.sleep(5000);
...
//            }
        ...
        protected void onPostExecute(List<Credential> credentials) {
            Log.d(TAG, "AliasLoader onPostExecute started");
