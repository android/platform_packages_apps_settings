...
public class ApnMimsiEditor extends SettingsPreferenceFragment
	implements OnPreferenceChangeListener, OnKeyListener {

    private final static String TAG = ApnMimsiEditor.class.getSimpleName();
    ...
    private Uri mTmpUri; // URI used temporary.

    class MccMncId { public int id, mcc, mnc; }

    ArrayList<MccMncId> mMccMncId = new ArrayList<MccMncId>();
    Hashtable<String, Integer> mNumericToId
	...
	addPreferencesFromResource(R.xml.apn_mimsi_editor);
	...
	mMccMnc = (EditTextPreference) findPreference("apn_mccmnc");
	...
	if (!action.equals("com.android.settings.apn.mimsi.action.EDIT") &&
		!action.equals("com.android.settings.apn.mimsi.action.INSERT")) {
	    finish();
	    return;
	}

	if (action.equals("com.android.settings.apn.mimsi.action.INSERT")) {
	    if (mFirstTime || icicle.getInt(SAVED_POS) == 0) {
		Uri uri = intent.getData();
		mTmpUri = getContentResolver().insert(Telephony.Carriers.CONTENT_URI, new ContentValues());
	    } else {
		mTmpUri = ContentUris.withAppendedId(Telephony.Carriers.CONTENT_URI,
			icicle.getInt(SAVED_POS));
	    }
	    mNewApn = true;
	    mCursor = getActivity().managedQuery(mTmpUri, sProjection, null, null);
	    mCursor.moveToFirst();
	}
	String apn = intent.getStringExtra("APN");
	if (apn != null) {
	   StringBuilder where = new StringBuilder("apn=\"" + apn + "\"");

	   mCursor = getContentResolver().query(Telephony.Carriers.CONTENT_URI, sProjection, where.toString(), null, Telephony.Carriers.DEFAULT_SORT_ORDER);
    ...
    * Disables given fields so that user cannot modify them
	...
	if (mFirstTime) {
		...
		while (!mCursor.isAfterLast()) {
		    int numeric = mCursor.getInt(NUMERIC_INDEX);
		    int mcc = mCursor.getInt(MCC_INDEX);
		    int mnc = mCursor.getInt(MNC_INDEX);
		    int id = mCursor.getInt(ID_INDEX);

		    MccMncId entry = new MccMncId();
		    entry.mcc = mcc;
		    entry.mnc = mnc;
		    entry.id = id;
		    mMccMncId.add(entry);
			...
			mccMncs = mccMncs + "," + numeric;
		    }
		    mNumericToId.put("" + numeric, id);
		    mCursor.moveToNext();
		}
		mMccMnc.setText(mccMncs);
	    }
	}
	...
	mMccMnc.setSummary(checkNull(mMccMnc.getText()));
	...
	if (validateAndSave(true) && mTmpUri != null) {
	...
	String mccMncs = checkNotSet(mMccMnc.getText());
	...
	for (String mccMnc: mccMncs.split(",")) {
	    ...
	    // Test if entry exist in hast table
	    if (mNumericToId.containsKey(mccMnc)) {

		int id = mNumericToId.get(mccMnc);
		Uri url = ContentUris.withAppendedId(Telephony.Carriers.CONTENT_URI, id);

		getContentResolver().update(url, values, null, null);
		Log.d(TAG, "validateAndSave: Update apn" + mccMnc);
		mNumericToId.remove(mccMnc);
	    } else {
		getContentResolver().insert(Telephony.Carriers.CONTENT_URI, values);
		Log.d(TAG, "validateAndSave: Insert new apn" + mccMnc);
	    }
	}
	Enumeration e = mNumericToId.keys();
	while (e.hasMoreElements()) {
	    String mccMnc = (String) e.nextElement();
	    int id = mNumericToId.get(mccMnc);
	    Uri url = ContentUris.withAppendedId(Telephony.Carriers.CONTENT_URI, id);
	    Log.d(TAG, "validateAndSave: Delete apn" + mccMnc);
	...
	if (name.length() < 1) {
	    ...
	    for (String mccMnc: mccMncs.split(",")) {
		if (mccMnc.length() != 5) {
		    errorMsg = mRes.getString(R.string.error_mcc_not5);
		    break;
		}
	    }
	}
	...
	// We must check delete apn for all mcc+mnc couple if exist before the edition
	String mccMncs = mMccMnc.getText();
	for (String mccMnc: mccMncs.split(",")) {
	    // Test if entry exist in hast table
	    if (mNumericToId.containsKey(mccMnc)) {
		int id = mNumericToId.get(mccMnc);
	...
	public static void showError(ApnMimsiEditor editor) {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
	    String msg = ((ApnMimsiEditor) getTargetFragment()).getErrorMsg();
