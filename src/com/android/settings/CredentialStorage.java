    ...
    private void installIfAvailable() {
        ...
        if (bundle.containsKey(Credentials.EXTRA_USER_PRIVATE_KEY_NAME)) {
            ...
            if (!mKeyStore.importKey(key, value, mInstallUid, flags)) {
                Log.e(TAG, "Failed to install " + key + " as mInstallUid " + mInstallUid);
                return;
            }
        }
        ...
        if (bundle.containsKey(Credentials.EXTRA_USER_CERTIFICATE_NAME)) {
            ...
            if (!mKeyStore.put(certName, certData, mInstallUid, flags)) {
                Log.e(TAG, "Failed to install " + certName + " as mInstallUid " + mInstallUid);
                return;
            }
        }
        ...
        if (bundle.containsKey(Credentials.EXTRA_CA_CERTIFICATES_NAME)) {
            ...
            if (!mKeyStore.put(caListName, caListData, mInstallUid, flags)) {
                Log.e(TAG, "Failed to install " + caListName + " as mInstallUid " + mInstallUid);
                return;
            }
        }
