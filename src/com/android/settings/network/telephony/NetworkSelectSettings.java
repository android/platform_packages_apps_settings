...
public class NetworkSelectSettings extends DashboardFragment {
            ...
            ThreadUtils.postOnBackgroundThread(() -> {
                ...
                if (!mShowSingleRowForOperator && mUseNewApi) {
                    msg.obj = mTelephonyManager.setNetworkSelectionModeManual(
                            ...
                            operatorInfo.getRan(), true /* persistSelection */);
                } else {
                    msg.obj = mTelephonyManager.setNetworkSelectionModeManual(
     ...
     * with the strongest signal strength if the operator needs to show single row.
     */
    private List<CellInfo> aggregateCellInfoList(List<CellInfo> cellInfoList) {
        // Old API doesn't contain cell type info, so show all networks only when using new API.
        if (!mShowSingleRowForOperator && mUseNewApi) {
            return cellInfoList;
        }
