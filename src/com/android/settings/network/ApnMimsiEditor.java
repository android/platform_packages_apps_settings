...
public class ApnMimsiEditor extends SettingsPreferenceFragment
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (!action.equals(mimsi_edit_action) &&
                !action.equals(mimsi_insert_action)) {
            finish();
            return;
        }

        if (action.equals(mimsi_insert_action)) {
    ...
    private void fillUi() {
        if (mFirstTime) {
            ...
            if (!mNewApn) {
                ...
                mMccMnc.setText(mccMncs.replace(" ", ""));
            }
        }
    ...
    private boolean validateAndSave(boolean force) {
        ...
        String mccMncs = checkNotSet(mMccMnc.getText()).replace(" ", "");
    ...
    private String getErrorMsg() {
        ...
        if (name.length() < 1) {
            ...
            for (String mccMnc : mccMncs.split(",")) {
                if (mccMnc.trim().length() != 5 && mccMnc.trim().length() != 6) {
    ...
    private void deleteApn() {
        ...
        String mccMncs = mMccMnc.getText().replace(" ", "");
        for (String mccMnc : mccMncs.split(",")) {
            // Test if entry exist in hast table
            if (mNumericToId.containsKey(mccMnc)) {
                int id = mNumericToId.get(mccMnc);
