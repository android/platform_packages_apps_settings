...
public class SubscriptionSettingUtility {
    ...
    //TODO:: Work around
    public static final String SLOT_KEY = "slotId";
    ...
    /**
     * Get Number of SIM card available
     * @param context The Context 
     * @return The Number of available SIM cards 
     */
    ...
    public static List<SubInfoRecord> getSortedSubInfoList(Context context) {
        List<SubInfoRecord> infoList = SubscriptionManager.getActivatedSubInfoList(context);
        if (infoList != null) {
            Collections.sort(infoList, new Comparator<SubInfoRecord>(){

                @Override
                public int compare(SubInfoRecord arg0, SubInfoRecord arg1) {
                    int flag = arg0.mSimId - arg1.mSimId;
                    if (flag == 0) {
                        return (int) (arg0.mSubId - arg1.mSubId);
                    }
                    return flag;
                }
            });
    ...
    private void handleSimStateChange(int simId, String state) {
        if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(state) && 
            SubscriptionManager.getSimId(mSubId) == simId) {
    ...
    public void initSubscriptionId(Bundle bundle, Intent intent) {
        ...
        if (mSubId == INVALID_ID) {
            ...
            if (mSubId == INVALID_ID) {
                List<SubInfoRecord> subInfoList = SubscriptionManager.getActivatedSubInfoList(mActivity);
                if (subInfoList != null) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                mSubId = data.getLongExtra(PhoneConstants.SUBSCRIPTION_KEY,INVALID_ID);
            } else {
                mActivity.onBackPressed();
            }
        } 
     ...
     */
    public int getSimId() {
        return SubscriptionManager.getSimId(mSubId);
    }
    ...
    /**
     * Get sub Id from intent with extra @PhoneConstants.SUBSCRIPTION_KEY
     * @param intent The intent that Sub id is gotten from
     * @return The Sub Id or -1
     */
    public long getSubIdByIntent(Intent intent) {
        if (intent != null) {
            return intent.getLongExtra(PhoneConstants.SUBSCRIPTION_KEY, INVALID_ID);
        }
        return INVALID_ID;
    }
