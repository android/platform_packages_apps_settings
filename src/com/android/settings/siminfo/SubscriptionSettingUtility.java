...
public class SubscriptionSettingUtility {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                mSubId = data.getLongExtra(PhoneConstants.SUBSRIPTION_KEY,INVALID_ID);
            } else {
                mActivity.onBackPressed();
            }
        } 
    ...
    /**
     * Get sub Id from intent with extra @PhoneConstants.SUBSRIPTION_KEY
     * @param intent The intent that Sub id is gotten from
     * @return The Sub Id or -1
     */
    public long getSubIdByIntent(Intent intent) {
        if (intent != null) {
            return intent.getLongExtra(PhoneConstants.SUBSRIPTION_KEY, INVALID_ID);
        }
        return INVALID_ID;
    }
