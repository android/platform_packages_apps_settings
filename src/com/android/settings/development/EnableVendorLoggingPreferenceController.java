...
public class EnableVendorLoggingPreferenceController extends DeveloperOptionsPreferenceController
    ...
    void setDeviceLoggingEnabled(boolean enable) {
        ...
        if (service == null || mDumpstateHalVersion < DUMPSTATE_HAL_VERSION_1_1) {
            if (DBG) Log.d(TAG, "setDeviceLoggingEnabled not supported.");
            return;
        }
        ...
        try {
            ...
            if (service11 != null) {
                service11.setDeviceLoggingEnabled(enable);
            }
        } catch (RemoteException | RuntimeException e) {
            if (DBG) Log.e(TAG, "setDeviceLoggingEnabled fail: " + e);
        }
    }
    ...
    boolean getDeviceLoggingEnabled() {
        ...
        if (service == null || mDumpstateHalVersion < DUMPSTATE_HAL_VERSION_1_1) {
            if (DBG) Log.d(TAG, "getDeviceLoggingEnabled not supported.");
            return false;
        }
        ...
        try {
            android.hardware.dumpstate.V1_1.IDumpstateDevice service11 =
                    (android.hardware.dumpstate.V1_1.IDumpstateDevice) service;
            if (service11 != null) {
                return service11.getDeviceLoggingEnabled();
            }
        } catch (RemoteException | RuntimeException e) {
            if (DBG) Log.e(TAG, "getDeviceLoggingEnabled fail: " + e);
        }
        return false;
    }
