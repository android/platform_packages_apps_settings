...
public class PlatformCompatDashboard extends DashboardFragment {
    ...
    public int getMetricsCategory() {
        return SettingsEnum.SETTINGS_PLATFORM_COMPAT_DASHBOARD;
    }
    ...
    private Map<Long, Boolean> getAppChangeMappings() {
        try {
    ...
    private Preference createPreferenceForChange(Context context, CompatibilityChangeInfo change,
        ...
        String changeName = change.name != null ? change.name : "Change_" + change.changeId;
    ...
    private void addChangePreferencesToCategory(List<CompatibilityChangeInfo> changes,
            PreferenceCategory category, Map<Long, Boolean> configMappings) {
    ...
    private void addPreferences() {
        ...
        // after target sdk.
        Map<Long, Boolean> configMappings = getAppChangeMappings();
        ...
        for (CompatibilityChangeInfo change : mChanges) {
            if (change.enableAfterTargetSdk != 0) {
                ...
                if (!targetSdkChanges.containsKey(change.enableAfterTargetSdk)) {
                    sdkChanges = new ArrayList<>();
                    targetSdkChanges.put(change.enableAfterTargetSdk, sdkChanges);
                } else {
                    sdkChanges = targetSdkChanges.get(change.enableAfterTargetSdk);
                }
