...
class ConfigDialog extends AlertDialog implements TextWatcher,
    ...
    protected void onCreate(Bundle savedState) {
        ...
        mType.setSelection(convertTypeToSelectedItemPosition(mProfile.type));
        ...
        if (mEditing) {
            ...
            changeType(convertTypeIndexToVpnProfileConstant(mProfile.type));
            ...
            setUsernamePasswordVisibility(convertTypeIndexToVpnProfileConstant(mProfile.type));
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        if (parent == mType) {
            changeType(convertTypeIndexToVpnProfileConstant(position));
    ...
    /**
     * Convert the VPN type selected by user to the VPN type defined in VpnProfile.java.
     *
     * It's a temporary function to convert the vpn type to the correct one because the code of
     * legacy VPN hasn't been removed yet from framework.
     */
    private int convertTypeIndexToVpnProfileConstant(int type) {
        switch (type) {
            case 0:
                return VpnProfile.TYPE_IKEV2_IPSEC_USER_PASS;
            case 1:
                return VpnProfile.TYPE_IKEV2_IPSEC_PSK;
            case 2:
                return VpnProfile.TYPE_IKEV2_IPSEC_RSA;
            default:
                return type;
        }
    }
    ...
    /**
     * Convert the type defined in VpnProfile.java to selected item position in spinner.
     */
    private int convertTypeToSelectedItemPosition(int type) {
        switch (type) {
            case VpnProfile.TYPE_IKEV2_IPSEC_USER_PASS:
                return 0;
            case VpnProfile.TYPE_IKEV2_IPSEC_PSK:
            ...
            case VpnProfile.TYPE_IKEV2_IPSEC_RSA:
                return 2;
            default:
                // If the type is out of range, then returning 0 as default value to prevent VPN
                // Settings crashed.
                return 0;
        }
    }
    ...
    private boolean validate(boolean editing) {
        ...
        final int type = convertTypeIndexToVpnProfileConstant(mType.getSelectedItemPosition());
    ...
    VpnProfile getProfile() {
        ...
        profile.type = convertTypeIndexToVpnProfileConstant(mType.getSelectedItemPosition());
