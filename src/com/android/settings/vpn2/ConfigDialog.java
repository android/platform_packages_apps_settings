...
class ConfigDialog extends AlertDialog implements TextWatcher,
    ...
    private void changeType(int type) {
        ...
        mView.findViewById(R.id.identifier).setVisibility(View.GONE);

        setUsernamePasswordVisibility(type);

        // Always enable identity for IKEv2/IPsec profiles.
        if (!isLegacyType(type)) {
            mView.findViewById(R.id.identifier).setVisibility(View.VISIBLE);
        }
    ...
    private void setUsernamePasswordVisibility(int type) {
        if (requiresUsernamePassword(type)) {
            mView.findViewById(R.id.userpass).setVisibility(View.VISIBLE);
        } else {
            mView.findViewById(R.id.userpass).setVisibility(View.GONE);
        }
    }
